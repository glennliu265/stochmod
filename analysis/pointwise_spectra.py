...
# -*- coding: utf-8 -*-
"""
Compute the spectra for a given dataset at each point
This is done for stochastic model output, or CESM1 preprocessed to match this
format

The first section calculates pointwise spectra
The second section investigates the spectra over specific regions
...
for f,sst_fn in enumerate(fnames):
    ...
    if len(sst.shape) < 4:
        ...
        sst = sst[None,...]
    nmod,nlon,nlat,ntime = sst.shape
    ...
    for mid in range(nmod):
        for o in tqdm(range(nlon)): # Loop for longitude
            ...
            for a in range(nlat): # Loop for latitude
                ...
                if np.any(np.isnan(sst[mid,o,a,:])):
                    continue
